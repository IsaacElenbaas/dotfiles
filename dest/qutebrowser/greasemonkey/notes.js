...
var text = document.children[0].innerText;
text = text.replace(/((?:\n\\`[^`]+\\`)+\n)/g, function(all, first) { return "\n```math" + first.replace(/\\`/g, "") + "```\n"; });
...
for(var i = 0; i < math.length; i++) {
	math[i].innerText = "`" + math[i].innerText + "`"; // Also adds <br>s which is nice because we can't process them from the node itself
	math[i].innerHTML = math[i].innerHTML.replace(/<br[^>]*>/gim, "`\n\n`");
}


var JaxConf = document.createElement("script");
JaxConf.innerText =`
	MathJax = {
		loader: {load: ["input/asciimath", "output/chtml"]},
		options: {
			skipHtmlTags: [],
			processHtmlClass: "math|language-math"
		}
	};
`; // clearing skipHtmlTags shouldn't be required as processHtmlClass should take priority, but it isn't for some reason
document.head.appendChild(JaxConf);

var JaxSrc = document.createElement('script');
JaxSrc.type = "text/javascript";
JaxSrc.src = 'file:///home/isaacelenbaas/dotfiles/dest/qutebrowser/MathJax/startup.js';
document.head.appendChild(JaxSrc);
...
console.log("Starting GitHub theme");
...
document.body.style.margin = "49px";
